...
namespace ASM.Server.Controllers
{
	...
	public class FoodController : ControllerBase
	{
		...
		public async Task<ActionResult<IEnumerable<FoodDto>>> GetAll()
		{
			var foods = await _db.Foods
                ...
                .Select(f => new FoodDto
				{
					...
					AverageRating = f.Reviews != null && f.Reviews.Count > 0
									? (int)Math.Round(f.Reviews.Average(r => r.Rating))
									: 0
				}).ToListAsync();
        ...
        public async Task<ActionResult<IEnumerable<FoodDto>>> GetById(int categoryId)
        {
            var food = await _db.Foods
                .Include(f => f.Category)
                .Include(f => f.Reviews)
                .Where(f => f.CategoryId == categoryId && f.DeletedAt == null)
                .Select(f => new FoodDto
                {
                    Id = f.Id,
                    Name = f.Name,
        ...
        /// </summary>
        /// <param name="id"></param>
        /// <returns>trả về thời gian món ăn bị xóa</returns>
